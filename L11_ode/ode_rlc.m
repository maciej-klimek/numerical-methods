...
% ODE Runge-Kutta 45 - funkcja Matlaba
  ...
  x0 = [ 0; 0 ];             % warunki początkowe: [uC(t(1)), i(t(1))] 
  fun = @(t,x) ( [ 1/C*x(2); ...                                 % pobudzenie:
                   1/L * (E                - R*x(2) - x(1)) ] ); % skok jednostkowy
...
% Porownanie z funkcją Matlaba ode45
  figure; plot( T, X(:,1),'ro-', t, x(:,1),'bx-'), title('uC(t): ode45 (o) i odeRK4AB');
...
% Porownanie z teoretyczna odpowiedzia na skok jednostkowy
  ...
  figure; plot(t, x(:,1),'bx--', t, ref(t,ksi,w0), 'ro--'); grid;
