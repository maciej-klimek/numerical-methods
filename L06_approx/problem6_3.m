...
x = [ 1 2 3 4 5 6 7 ];
y = [ 0.912 0.945 0.978 0.856 1.013 1.035 1.057];

figure; plot( x, y, 'bo' ); title('y=f(x)'); grid; pause;
...
figure; plot( x, y, 'bo', x, a*x+b, 'k-' ); title('y=f(x)'); grid; pause;
% Takze wielomiany wyzszych rzedow
figure;
p = polyfit( x, y, 2 ), % a=p(1), b=p(2)
subplot(221); plot( x, y, 'bo', x, polyval(p,x), 'r-' ); title('Stopień wielomianu: 2'); grid;
p = polyfit( x, y, 3 ), % a=p(1), b=p(2)
subplot(222); plot( x, y, 'bo', x, polyval(p,x), 'r-' ); title('Stopień wielomianu: 3'); grid;
p = polyfit( x, y, 4 ), % a=p(1), b=p(2)
subplot(223); plot( x, y, 'bo', x, polyval(p,x), 'r-' ); title('Stopień wielomianu: 4'); grid;
p = polyfit( x, y, 6 ), % a=p(1), b=p(2)
subplot(224); plot( x, y, 'bo', x, polyval(p,x), 'r-' ); title('Stopień wielomianu: 5'); grid;