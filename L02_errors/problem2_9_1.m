...
if(0) % Metoda #1
  ...
  num2bitstr(single(x)); pause  % pokaz bity
  while( x < realmax )    % jesli x mniejsze od wartosci najwiekszej, to ... 
     ...
     num2bitstr(single(x));     % pokaz bity
     pause                % analizuj je
...
if(1) % Metoda #2: x=1, mnozenie razy 2 #################################
  ...
  while(2*x > x)              % czy widac mnozenie przez 2; jesli tak, to 
     ...
     num2bitstr(single(x));         % zobacz zmiane bitow reprezentacji
...
if(0) % Metoda #3: x=1, dzielenie przez 2
  ...
  num2bitstr(single(x)); pause      % zobacz reprezentacje bitowa
