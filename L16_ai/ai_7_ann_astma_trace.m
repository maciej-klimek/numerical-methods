...
% Trenowanie sztucznej sieci neuronowej
  ...
  if(1)
     ...
     procent_ucz = 75;            % ile procent danych do uczenia, reszta do rozpoznawania
     nr = randperm( L_wzorcow );  % opcjonalnie
     L_ucz = ceil( procent_ucz / 100 * L_wzorcow );
     nu = [ nr(1 : L_ucz), L_wzorcow + nr(1 : L_ucz) ]; 
     nw = [ nr(L_ucz+1 : L_wzorcow), L_wzorcow + nr(L_ucz+1 : L_wzorcow) ];
  end
  Nu = length( nu ),
  Nw = length( nw ), pause

  Xu = X(nu,:);      % u = dane do uczenia
  yu = y(nu);        %
  Xw = X(nw,:);      % w = dane do walidacji
  yw = y(nw);        %
  ...
  uchwyt_funkcji = @(p) ann_koszt_gradient(p, param_warstw, Xu, yu, lambda );    % uchwyt
  [ Wagi_wektor, koszt ] = fmincg_trace( uchwyt_funkcji, Wagi_wektor, optim_opcje ); % optymal.
...
% Rozpoznawanie z uzyciem nauczonej sieci
  ...
  fprintf('Accuracy: %f\n', mean(double( y_pred == yw )) * 100);    % dokladnosc rozpoznania w [%]
  figure;
  indx = 1 : length(yw);
  plot( indx, yw,'r-', indx, y_pred,'bx','LineWidth',2); grid; title('Wyniki rozpoznawania');
  xlabel('nr testu'); ylabel('rozpoznana wartosc');
  legend('zadane','rozpoznane','Location','southeast');

% Analiza wynikow  
  C1 = hist( y_pred( indx <= Nw/2 ), 1 : L_klas );
  C2 = hist( y_pred( indx >  Nw/2 ), 1 : L_klas );
